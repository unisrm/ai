...
bool permutation(int count, node* nodeList, int n, string s1, string s2, string s3){ if (n == count - 1){  
for (int i = 0; i < 10; i++){ 
if (use[i] == 0){  
nodeList[n].value = i;  
...
cout << " " << nodeList[j].letter << " = "<< nodeList[j].value<<endl; return true; 
} 
}
} 
return false; 
...
nodeList[n].value = i;  
use[i] = 1; 
if (permutation(count, nodeList, n + 1, s1, s2, s3))  return true; 
use[i] = 0;  
} 
} 
return false; 
} 
bool solvePuzzle(string s1, string s2,string s3){ int uniqueChar = 0;  
int len1 = s1.length(); 
int len2 = s2.length(); 
int len3 = s3.length(); 
vector<int> freq(26); 
